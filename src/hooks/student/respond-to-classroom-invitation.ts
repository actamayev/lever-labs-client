...
export default function useRespondToClassroomInvitation() : (
	classCode: ClassCode,
	inviteResponse: "accept" | "decline"
) => Promise<boolean> {
	const navigate = useTypedNavigate()

	return useCallback(async (classCode: ClassCode, inviteResponse: "accept" | "decline"): Promise<boolean> => {
		try {
			...
			if (classroomData) {
				...
				// Remove the classroom from the student's data if declined
					...
					const remainingClassrooms = studentClass.removeClassroomData(classCode)
					// eslint-disable-next-line max-depth
					if (remainingClassrooms === 0) navigate("/garage")
				}
			}
