...
// Simple version that only triggers ENTER on mount (with optional delay) and EXIT on unmount/page hide/refresh.
// No pip connection monitoring, no intervals, no complex state management.
export default function useSimpleCareerQuestTrigger(
	...
	// Effect to handle enter/exit triggers
	useEffect((): (() => void) => {
		if (!enabled) return (): void => {}

		// Send enter trigger after delay
		const triggerTimeout = setTimeout((): void => {
			if (!enterTrigger || hasExitedRef.current) return
