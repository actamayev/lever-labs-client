...
export default function useMotorDemoUseEffect(): void {
	...
	const handleMotorControl = useCallback(async (leftMotor: MotorDirection, rightMotor: MotorDirection): Promise<void> => {
		try {
			...
			const motorControlResponse = await blueDotApiClient.labDemoDataService.motorControl(newMotorState)
			if (!isEqual(motorControlResponse.status, 200) || isNonSuccessResponse(motorControlResponse.data)) {
