...
const RainbowSubscribeButton: React.FC<RainbowSubscribeButtonProps> = ({
  ...
  const rainbowButtonClasses = cn(
    "group relative inline-flex h-11 animate-rainbow items-center justify-center rounded-md border-0 bg-[length:200%] px-8 py-2 font-medium text-primary-foreground transition-colors [background-clip:padding-box,border-box,border-box] [background-origin:border-box] [border:calc(0.08*1rem)_solid_transparent] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring",
    ...
    <AnimatePresence mode="wait">
      {isSubscribed ? (
        <motion.button
          type="button"
          disabled={true}
          className="relative flex w-48 items-center justify-center overflow-hidden rounded-md bg-white p-2.5 outline outline-1 outline-black"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
