...
export default function ActivityProgressIconContainer({ icon, activityProgressPercent }: Props) {
	// Ensure progress is between 0 and 100
	const progress = Math.min(100, Math.max(0, activityProgressPercent))
		...
		<TooltipProvider delayDuration={0}>
			<Tooltip>
				...
				<TooltipContent side="bottom" className="text-zinc-100 dark:text-zinc-900 mt-2">
					{Math.round(activityProgressPercent)}% complete
				</TooltipContent>
			</Tooltip>
		</TooltipProvider>
	)
